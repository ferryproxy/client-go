...
// List lists all Hubs in the indexer.
func (s *hubLister) List(selector labels.Selector) (ret []*v1alpha2.Hub, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.Hub))
	})
	return ret, err
}
...
// List lists all Hubs in the indexer for a given namespace.
func (s hubNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.Hub, err error) {
...
// Get retrieves the Hub from the indexer for a given namespace and name.
func (s hubNamespaceLister) Get(name string) (*v1alpha2.Hub, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("hub"), name)
	}
	return obj.(*v1alpha2.Hub), nil
}
